...
public class Stage2KaleidoscopeShader implements BackgroundShader {
    ...
    private static final String SOURCE_FRAGMENT_SHADER =
        "#ifdef GL_ES\n" +
        ...
        "}\n" +
        "\n" +
        "vec3 phosphene(in vec2 uv) {\n" +
        "    float angle = u_time * 0.0125;\n" +
        "    for (int i = 0; i < 32; i++) {\n" +
        "        uv = abs(uv);\n" +
        "        uv -= angle;\n" +
        "        uv.x -= angle;\n" +
        ...
        "    }\n" +
        "\n" +
        "    return palette(uv[1], vec3(0.480, 0.480, 0.480), vec3(0.200, 0.200, 0.200), vec3(6.000, 6.000, 6.000), vec3(2.028, 2.028, 2.028));\n" +
        "}\n" +
        "\n" +
        "vec3 tentacles(in vec2 uv) {\n" +
        "    vec3 ro = vec3(0, 0, -3);\n" +
        ...
        "        vec3 p = ro + rd * t;\n" +
        "\n" +
        "        p.xy *= rot2D(t * sin(u_time * 0.05) * m.x);\n" +
        "\n" +
        "        p.y += sin(t) * 0.35;\n" +
        "\n" +
        "        float d = map(p);\n" +
        ...
        "\n" +
        "    vec3 finalPalette;\n" +
        "    if (u_time < u_btime) finalPalette = phosphene(uv);\n" +
        "    else finalPalette = tentacles(uv);\n" +
        "\n" +
        "    gl_FragColor = vec4(finalPalette, 1.0);\n" +
        "}\n";

    // Ported from the guide's Buf A. u_channel0 = this frame's source render (currentSource()),
    // u_channel1 = LAST FRAME'S source render (previousSource()) - see the class doc for why
    // comparing true temporally-adjacent frames of the same content (not the accumulated/warped
    // buffer) is what gives a coherent motion field instead of noise. Outputs one motion vector per
    // pixel, biased +0.5 so negative offsets fit in an unsigned color channel (undone in
    ...
    private static final String MOTION_FRAGMENT_SHADER =
        ...
        "const int BLOCK_SAMPLE_STRIDE = " + BLOCK_SAMPLE_STRIDE + ";\n" +
        "const int ITERS = " + ITERS + ";\n" +
        "const int STRIDE = " + STRIDE + ";\n" +
        "\n" +
        "vec2 dim;\n" +
        "\n" +
        // Sparse-sampled - see the class/field docs on BLOCK_SAMPLE_STRIDE.
        "float blockDist(vec2 curUV, vec2 prevUV) {\n" +
        "    float dist = 0.0;\n" +
        "    for (int i = 0; i < BLOCK_DIM; i += BLOCK_SAMPLE_STRIDE) {\n" +
        "        for (int j = 0; j < BLOCK_DIM; j += BLOCK_SAMPLE_STRIDE) {\n" +
        "            vec2 offset = vec2(float(i) * dim.x, float(j) * dim.y);\n" +
        "            dist += distance(texture2D(u_channel0, curUV + offset), texture2D(u_channel1, prevUV + offset));\n" +
        "        }\n" +
        "    }\n" +
        "    return dist;\n" +
        "}\n" +
        "\n" +
        "void main() {\n" +
        // FrameBuffer textures come out bottom-row-first - see the class doc.
        "    vec2 flipped = vec2(v_texCoords.x, 1.0 - v_texCoords.y);\n" +
        "    dim = 1.0 / u_resolution;\n" +
        // This (deliberately small - see class doc) buffer has one pixel per BLOCK_DIM x BLOCK_DIM
        // source block; scale so the last row/column's block footprint doesn't sample past uv 1.0.
        "    vec2 maxOrigin = vec2(1.0) - float(BLOCK_DIM) * dim;\n" +
        "    vec2 uv = flipped * maxOrigin;\n" +
        "\n" +
        // A moderate zero-motion bias - phosphene/tentacles are high-frequency procedural patterns
        // (phosphene in particular iterates uv*=1.065 32 times, which blows small per-pixel
        // differences up exponentially), so neighboring candidate blocks routinely look about as
        // different from each other as they do from a genuinely bad match. An earlier version of this
        // used -20 (essentially always picking zero motion) to kill a full-image tearing bug, but that
        // also killed the actual datamosh look - the target effect (see the project's reference gif)
        // is macroblock-style chunks visibly streaking/displacing with hard edges between them, which
        // needs blocks to actually diverge from each other. The full-image tearing turned out to be
        // caused by the buffers including the wide blank HUD-panel margins around the real content
        // (see ensureFrameBuffers()/the tight-canvas projection in render()) - blocks right at a real-
        // content/blank-margin edge would find wildly "better" matches by shifting away from that
        // edge, and that motion compounded every frame. With the buffers now tightly cropped to just
        // the real content (no such edge exists anymore), a much smaller bias is safe again.
        "    float minDist = blockDist(uv, uv) - 2.0;\n" +
        "    vec2 minPointer = vec2(0.0);\n" +
        "\n" +
        "    vec2 dirs[8];\n" +
        "    dirs[0] = vec2(0.0, 1.0);\n" +
        "    dirs[1] = vec2(1.0, 1.0);\n" +
        ...
        "    }\n" +
        "\n" +
        "    gl_FragColor = vec4(minPointer + 0.5, 0.0, 1.0);\n" +
        "}\n";

    // Ported from the guide's Buf C. u_motion = this frame's estimated motion field, u_previous =
    // this same accumulate buffer's own previous frame (the ping-pong "read" side), u_source =
    // this frame's actual source render. u_warmup mirrors the guide's "iFrame < 5" guard. u_decay is
    // computed fresh each frame in render() (a ramp, not the DECAY constant directly) - see the class
    // doc. Sampled per-pixel (smooth), not snapped to the motion buffer's block grid - the target
    // look is the old image visibly dragged/distorted by real motion, not a hard-edged block mosaic.
    private static final String ACCUMULATE_FRAGMENT_SHADER =
        "#ifdef GL_ES\n" +
        "precision highp float;\n" +
        "#endif\n" +
        "varying vec2 v_texCoords;\n" +
        ...
        "        return;\n" +
        "    }\n" +
        "    vec2 pointer = texture2D(u_motion, uv).xy - 0.5;\n" +
        "    vec4 warped = texture2D(u_previous, uv + pointer);\n" +
        "    vec4 fresh = texture2D(u_source, uv);\n" +
        "    gl_FragColor = warped * u_decay + fresh * (1.0 - u_decay);\n" +
        "}\n";
    ...
    public Stage2KaleidoscopeShader() {
        ...
        sourceShader = new ShaderProgram(VERTEX_SHADER, SOURCE_FRAGMENT_SHADER);
        if (!sourceShader.isCompiled()) {
            throw new IllegalStateException("Stage2KaleidoscopeShader source pass failed to compile: " + sourceShader.getLog());
        }
        motionShader = new ShaderProgram(VERTEX_SHADER, MOTION_FRAGMENT_SHADER);
        if (!motionShader.isCompiled()) {
            throw new IllegalStateException("Stage2KaleidoscopeShader motion pass failed to compile: " + motionShader.getLog());
        }
        accumulateShader = new ShaderProgram(VERTEX_SHADER, ACCUMULATE_FRAGMENT_SHADER);
        if (!accumulateShader.isCompiled()) {
            throw new IllegalStateException("Stage2KaleidoscopeShader accumulate pass failed to compile: " + accumulateShader.getLog());
        }
    }
